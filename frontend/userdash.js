...
// Global premium confirm helper
function showConfirm({ title, message, warning, icon, confirmText, onConfirm }) {
    ...
    m.innerHTML = `
        <div style="background:#fff;border-radius:20px;padding:30px 24px;max-width:380px;width:90%;box-shadow:0 25px 60px rgba(0,0,0,0.18);text-align:center;">
            ...
            <h3 style="margin:0 0 8px;font-size:18px;font-weight:800;color:#111827;">${title || 'Are you sure?'}</h3>
            <p style="margin:0 0 ${warning ? '12' : '22'}px;font-size:13px;color:#4b5563;line-height:1.6;">${message || ''}</p>
            ${warning ? `<p style="background:#fef2f2;color:#991b1b;border-radius:10px;padding:10px 14px;font-size:12px;font-weight:600;margin-bottom:22px;">${warning}</p>` : ''}
            <div style="display:flex;gap:10px;">
                <button id="_cCancel" style="flex:1;padding:11px;border-radius:30px;border:1.5px solid #e5e7eb;background:#fff;font-size:13px;font-weight:600;color:#6b7280;cursor:pointer;">Cancel</button>
                <button id="_cOk" style="flex:1;padding:11px;border-radius:30px;border:none;background:#800000;font-size:13px;font-weight:700;color:#fff;cursor:pointer;box-shadow:0 4px 12px rgba(128,0,0,0.25);">${confirmText || 'Confirm'}</button>
            </div>
        </div>`;
    document.body.appendChild(m);
    const close = () => m.remove();
    document.getElementById('_cCancel').onclick = close;
    m.addEventListener('click', e => { if (e.target === m) close(); });
    document.getElementById('_cOk').onclick = () => { close(); onConfirm(); };
}
...
function injectAccountModal() {
    ...
    const modalHTML = `
    <div id="openAccountModal" class="modal" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);z-index:9999;justify-content:center;align-items:center;">
        <div class="card" style="width:100%;max-width:450px;margin:20px;animation:slideIn 0.3s ease;max-height:90vh;overflow-y:auto;">
            <div class="card-header" style="display:flex;justify-content:space-between;align-items:center;">
                <h3 class="card-title" style="margin:0;">Open New Account & KYC</h3>
                <button onclick="closeAccountModal()" style="background:none;border:none;font-size:20px;cursor:pointer;color:var(--text-secondary);">&times;</button>
            </div>
            <div style="padding:24px;">
                <p style="color:var(--text-secondary);margin-bottom:20px;font-size:14px;">Select the account type and complete Indian KYC (Aadhaar & PAN).</p>
                <div class="form-group">
                    <label class="form-label">Account Type</label>
                    <select id="newAccountType" class="form-select">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="kycAadhaar" class="form-control" placeholder="1234 5678 9012" maxlength="14" style="letter-spacing:1px;">
                </div>
                <div class="form-group">
                    <label class="form-label">PAN Number</label>
                    <input type="text" id="kycPan" class="form-control" placeholder="ABCDE1234F" maxlength="10" style="text-transform:uppercase;">
                </div>
                <div class="form-group">
                    <label class="form-label">Aadhaar Card Proof (Front)</label>
                    <input type="file" id="kycAadhaarProof" class="form-control" accept="image/*,.pdf">
                </div>
                <div class="form-group">
                    <label class="form-label">PAN Card Proof (Front)</label>
                    <input type="file" id="kycPanProof" class="form-control" accept="image/*,.pdf">
                </div>
                <div style="background:#f1f5f9;border-left:4px solid #3b82f6;padding:12px;margin:16px 0;font-size:13px;color:#334155;">
                    <i class="fas fa-camera" style="color:#3b82f6;margin-right:8px;"></i>
                    Face verify will start automatically when you click submit.
                </div>
                <div style="display:flex;gap:12px;margin-top:24px;">
                    <button class="btn" style="flex:1;background:var(--border-color);color:var(--text-primary);" onclick="closeAccountModal()">Cancel</button>
                    <button class="btn btn-primary" style="flex:1;" onclick="submitNewAccount()" id="btnSubmitAccount">Verify & Submit</button>
                </div>
            </div>
        </div>
    </div>`;
...
function showAccountModal() {
    // Filter out already owned account types or pending requests
    const dropdown = $id('newAccountType');
    if (dropdown) {
        ...
        Array.from(dropdown.options).forEach(opt => {
            ...
            if (owned.includes(val)) {
                opt.disabled = true;
                opt.text = `${opt.value} Account (Already Owned)`;
            } else if (pending.includes(val)) {
                opt.disabled = true;
                opt.text = `${opt.value} Account (Pending Approval)`;
            } else {
                opt.disabled = false;
...
function renderStats() {
    ...
    const balEl = $id('statTotalBalances');
    if (balEl) {
        balEl.textContent = isHidden ? '₹∗∗∗∗∗∗' : fmtINR(totalBalance);
    }
    ...
    setText('statTotalIncome', fmtINR(income));
    setText('statTotalSpending', fmtINR(spending));
    ...
    if (balanceContainer) {
        if (state.accounts.length === 0) {
            ...
            balanceContainer.style.display = 'block'; // Or 'flex' if it was originally flex
            if (state.accounts.length === 1) {
                const acc = state.accounts[0];
                if (balanceLabel) balanceLabel.innerHTML = `<i class="fas fa-wallet" style="margin-right: 8px; color: var(--primary-blue);"></i>${acc.account_type || 'Account'} Balance`;
...
function renderInvoiceTable() {
    ...
    tbody.innerHTML = txns.map(t => `<tr>
        ...
        <td style="padding:12px;"><span class="badge badge-${t.type === 'debit' ? 'success' : 'danger'}">${t.type.toUpperCase()}</span></td>
        <td style="padding:12px;">${t.mode || 'NEFT'}</td>
        <td style="padding:12px;font-weight:700;" class="${t.type === 'debit' ? 'text-success' : 'text-danger'}">
            ${t.type === 'debit' ? '+' : '-'}${fmtINR(t.amount)}</td>
    </tr>`).join('');
}
...
function txnRowHTML(t) {
    ...
    return `<div class="txn-item">
        ...
        <div class="txn-info">
            ...
            <p style="display:flex;gap:8px;flex-wrap:wrap;font-size:12px;color:var(--text-secondary);">
        ...
        <div class="txn-amount" style="text-align:right;">
            ...
            <button onclick="reportTransaction('${t.id}', '${t.reference_number || 'N/A'}', '${t.amount}')" style="margin-top:4px;background:none;border:none;color:#ef4444;font-size:11px;cursor:pointer;opacity:0.8;"><i class="fas fa-flag"></i> Report</button>
        </div>
    </div>`;
}
            ...
            <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px;">
                <div style="display:flex;align-items:center;gap:12px;">
                    <div class="txn-icon credit" style="flex-shrink:0;">
                        <i class="fas ${a.account_type?.toLowerCase().includes('saving') ? 'fa-piggy-bank' : 'fa-briefcase'}"></i>
                    </div>
                    <div>
...
async function requestAccountConversion(id, currentType) {
    const newType = prompt(`Convert your ${currentType} account to which type? (Savings / Current / Salary)`);
    if (!newType) return;
    const nt = newType.trim().toLowerCase();
    const valid = { 'savings': 'Savings', 'current': 'Current', 'salary': 'Salary' };
    ...
    let taxId = null;
    if (valid[nt] === 'Current') {
        taxId = prompt("Tax ID is required for Current Accounts. Please enter your Tax ID:");
        if (!taxId || taxId.trim() === '') {
        ...
        <div style="padding:24px;">
            ...
            <div id="panel_IMPS" class="transfer-panel" style="display:none;">
                ...
                <div class="form-group"><label class="form-label">Beneficiary Account Number</label>
                    <input id="impsToAcct" class="form-input" placeholder="Account number"></div>
            ...
            <div id="panel_NEFT" class="transfer-panel" style="display:block;">
                ...
                <div class="form-group"><label class="form-label">Beneficiary Account Number</label>
                    <input id="neftToAcct" class="form-input" placeholder="Account number"></div>
                <div class="form-group"><label class="form-label">IFSC Code</label>
...
async function doTransfer(mode) {
    ...
    const confirmMsg = mode === 'UPI'
        ...
        : `Transfer ${fmtINR(amount)} to A/C ${maskAcct(toVal)}${bName ? ' (' + bName + ')' : ''} via ${mode}?`;
    ...
    await new Promise((resolve, reject) => {
        ...
        // Track cancel by watching modal removal
        const check = setInterval(() => {
            if (!document.getElementById('_confirmModal')) { clearInterval(check); }
        }, 200);
    }).catch(() => { return; });
...
function renderCards() {
    ...
    if (!state.cards.length) {
        el.innerHTML = `<div style="text-align:center;padding:3rem;">
            ...
            <p style="color:#6b7280;margin-bottom:1rem">No active cards yet</p>
    ...
    el.innerHTML = state.cards.map(c => {
        ...
        return `<div style="background:linear-gradient(135deg,${isCr ? '#6366f1,#8b5cf6' : '#1e3a8a,#1d4ed8'});color:white;border-radius:16px;padding:24px;margin-bottom:16px;min-height:150px;position:relative;">
            ...
            </div><span style="background:rgba(255,255,255,.2);padding:4px 10px;border-radius:20px;font-size:12px;height:fit-content;">${c.status}</span></div>
...
async function blockCard(cardId) {
    showConfirm({
        ...
        warning: '⚠ This action cannot be undone online. Contact support to re-activate.',
        icon: 'fa-ban',
...
function renderCardRequestsList() {
    ...
    el.innerHTML = `<table style="width:100%;border-collapse:collapse;">
        <thead><tr style="background:var(--bg-light);"><th style="padding:10px;font-size:12px;">Type</th>
        <th style="padding:10px;font-size:12px;">Requested</th><th style="padding:10px;font-size:12px;">Status</th></tr></thead>
        <tbody>${state.cardRequests.map(r => `<tr style="border-bottom:1px solid var(--border-color);">
            <td style="padding:12px;">${r.card_type} Card</td>
            <td style="padding:12px;font-size:13px;">${formatDate(r.request_date)}</td>
            <td style="padding:12px;"><span class="loan-status ${r.status}">${r.status}</span></td>
        </tr>`).join('')}</tbody></table>`;
}
...
function openLoanRepayModal(loanId, loanType, outstanding, emi) {
    ...
    $id('repayLoanInfo').innerHTML = `Repaying for <strong>${loanType}</strong> (Loan #${loanId}).<br>Outstanding amount: <strong>${fmtINR(outstanding)}</strong>`;

    // Populate source accounts
    const sel = $id('repaySourceAccount');
    if (sel) {
        sel.innerHTML = state.accounts.map(a => `<option value="${a.id}">${a.account_type} - ${maskAcct(a.account_number)} (${fmtINR(a.balance)})</option>`).join('');
    }
...
async function renderSupportPage() {
    ...
    el.innerHTML = `<div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;">
        ...
        <tbody>${requests.map(r => `<tr style="border-bottom:1px solid var(--border-color);">
            ...
            <td style="padding:10px;text-transform:capitalize;">${r.message && r.message.substring(0, 20)}...</td>
...
async function submitSupportRequest(event) {
    event.preventDefault();
    const req = {
        priority: $id('supportPriority')?.value, subject: $id('supportSubject')?.value, message: $id('supportMessage')?.value
    };
    try {
        ...
        showToast('Error connecting to support system', 'error');
    }
    ...
    list.innerHTML = state.notifications.map(n => `
        <div onclick="markNotifRead(${n.id})" style="padding:12px 16px;cursor:pointer;border-bottom:1px solid var(--border-color);${n.read ? '' : 'background:rgba(79, 70, 229, 0.05)'}">
            <div style="display:flex;gap:10px;">
                ...
                <div><div style="font-weight:600;font-size:13px;">${n.title}</div>
...
async function applyForLoan(type) {
    ...
    $id('loanTypeDisplay').value = type.charAt(0).toUpperCase() + type.slice(1) + ' Loan';

    // Populate disbursement accounts
    const accSel = $id('loanAccount');
    if (accSel) {
        accSel.innerHTML = state.accounts.map(a => `<option value="${a.id}">${a.account_type} — ${maskAcct(a.account_number)} (${fmtINR(a.balance)})</option>`).join('');
    }
...
function showToast(msg, type = 'info') {
    ...
    if (!t) {
        ...
        t.style.cssText = 'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);z-index:9999;padding:12px 24px;border-radius:12px;font-weight:600;font-size:14px;box-shadow:0 8px 24px rgba(0,0,0,.15);transition:opacity .3s;opacity:0;pointer-events:none;';
        document.body.appendChild(t);
    }
    const colors = { success: '#10b981', error: '#ef4444', info: '#3b82f6', warning: '#f59e0b' };
    t.style.background = colors[type] || colors.info; t.style.color = '#fff'; t.textContent = msg; t.style.opacity = '1';
...
async function submitUpiTransfer(event) {
    ...
    try {
        const r = await fetch(`${API}/transfer/upi`, {
            ...
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
            body: JSON.stringify({ target_vpa: vpa, amount: amt, upi_pin: pin })
        });
        const d = await r.json();
        if (r.ok && d.success) {
            showToast(`✓ Payment Successful! Ref: ${d.reference}`, 'success');
            $id('upiTransferForm').reset();
            addNotification('UPI Payment', `₹${amt} sent to ${vpa}`, 'success');
            await refreshBalance(true);
        } else {
